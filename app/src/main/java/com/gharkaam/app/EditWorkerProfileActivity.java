...
public class EditWorkerProfileActivity extends Activity {
    ...
    private GradientDrawable box(int color) {
        ...
        g.setStroke(dp(1), Color.rgb(220, 220, 230));
        return g;
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TextView workTypeLabel = label("Work Type");
        content.addView(workTypeLabel);

        workTypeSpinner = new Spinner(this);
        workTypeSpinner.setBackground(box(Color.WHITE));
        ...
        workTypeSpinner.setPadding(dp(16), 0, dp(16), 0);
        ...
        workType = new EditText(this);
        workType.setVisibility(View.GONE);

        content.addView(workType);
        ...
        FirebaseFirestore.getInstance()
                ...
                .addOnSuccessListener(snapshot -> {
                    ...
                    String existingWorkType =
                            normalizeWorkType(
                                    safe(doc.getString("workType"))
                            );

                    int workPosition = 0;

                    for (int i = 0; i < workTypes.length; i++) {
                        if (workTypes[i].equalsIgnoreCase(existingWorkType)) {
                            workPosition = i;
                            break;
                        }
                    }

                    workTypeSpinner.setSelection(workPosition);
                    workType.setText(existingWorkType);

                    experience.setText(
                            safe(doc.getString("experience"))
                    );
        ...
        save.setOnClickListener(v -> {
            ...
            String newWork =
                    workTypeSpinner.getSelectedItem().toString().trim();
